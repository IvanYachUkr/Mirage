...
def _genre_expansion_candidates(row: dict, universal_qualifiers: list[str], universal_contexts: list[str], rng: random.Random) -> deque[str]:
    ...
    for seed in seeds:
        ...
        for tone in tone_tokens[:8]:
            for context in contexts[:6]:
                candidates.append(f"{tone}-{context}-{seed}")
        for qualifier in qualifiers[:6]:
            for tone in tone_tokens[:6]:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Keyword generator for test30")
