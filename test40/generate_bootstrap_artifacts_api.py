...
from contracts import CERTIFICATIONS, COUNTRIES, GENRES, MARKETS, MODEL_TIERS, NATIONALITIES, PRODUCTION_TIERS, RELATIONSHIP_TARGETS
...
def _artifact_specs() -> dict[str, dict[str, Any]]:
    return {
        "identity_bank": {
            ...
            "default_model": MODEL_TIERS.get("artifact_mid", "gemini-3.1-flash-lite-preview"),
            "temperature": 0.35,
            "max_tokens": 18000,
        },
        "character_identity_bank": {
            "path": character_identity_bank_path,
            "default_model": MODEL_TIERS.get("artifact_pro", "gemini-3.1-pro-preview"),
            "temperature": 0.45,
            "max_tokens": 18000,
        },
        "company_lexicon": {
...
def _normalize_financial_priors(block: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(cert_dist, dict):
        ...
        for raw_genre, raw_row in cert_dist.items():
            ...
            if row_out:
                existing = normalized_cert_dist.get(genre_key, {})
    ...
    if isinstance(market_regime, dict):
        regime_block = dict(market_regime)
    ...
    if isinstance(award_weights, dict):
        weight_floors = {
        ...
        for key, floor_value in weight_floors.items():
            raw_value = award_weights.get(key, floor_value)
            try:
                normalized_weights[key] = max(float(floor_value), float(raw_value))
            except Exception:
                normalized_weights[key] = float(floor_value)
        for key, (lo, hi) in penalty_bounds.items():
            raw_value = award_weights.get(key, lo)
            try:
                normalized_weights[key] = min(float(hi), max(float(lo), float(raw_value)))
            except Exception:
...
def _runtime_schema_completion_skeleton(sections: tuple[str, ...]) -> dict[str, Any]:
    ...
    if "financial_priors" in sections:
        root["financial_priors"] = {
            ...
            "budget_ranges_by_tier": {
                str(tier): {"min_budget": 0.0, "max_budget": 1.0}
                for tier in PRODUCTION_TIERS
            },
            "certification_distribution_by_genre": {
                str(genre): {str(cert): 0.0 for cert in CERTIFICATIONS}
                for genre in GENRES
            },
            "company_profile_tiers": {
            ...
            "award_campaign_weights": {
                ...
                "director_ambition": 0.0,
                "director_reputation": 0.0,
                "cast_reputation": 0.0,
                "company_focus": 0.0,
                "q4_bonus": 0.0,
                "prestige_genre_bonus": 0.0,
                "regime_prestige_bias": 0.0,
                "director_momentum": 0.0,
                "company_momentum": 0.0,
                "graph_synergy": 0.0,
                "quality_signal": 0.0,
                "slate_pressure_penalty": 0.0,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate bootstrap artifacts for test30")
