...
Targets:
  ...
  prepare-reuse-seed      Optional: seed this run dir from older entities before step 80.
...
Key environment variables:
  ...
  ARTIFACT_SOURCE_DIR / ENTITY_SOURCE_DIR / EXTRA_TITLE_SOURCE_DIR configure prepare-reuse-seed.
EOF
}
...
require_seed_replace_ok() {
  if [[ -d "$BASE_DIR/entities" || -e "$BASE_DIR/movie.csv" || -e "$BASE_DIR/movie.arrow" || -d "$BASE_DIR/_step100_resume" ]]; then
    if [[ "${ALLOW_OVERWRITE:-0}" != "1" ]]; then
      echo "Refusing to replace existing entities or step-100 outputs in $BASE_DIR." >&2
      echo "Create a fresh run directory, or set ALLOW_OVERWRITE=1 only when this directory is disposable." >&2
      exit 3
    fi
  fi
}
...
case "$target" in
  ...
  prepare-reuse-seed)
    ...
    write_active_local_provider_env
    require_seed_replace_ok
    artifact_source="${ARTIFACT_SOURCE_DIR:-$BASE_DIR}"
    default_entity_source="$BASE_DIR/../test23"
    if [[ ! -d "$default_entity_source/entities" ]]; then
      default_entity_source="$BASE_DIR/../test32"
    fi
    default_extra_title_source="$BASE_DIR/../test35"
    entity_source="${ENTITY_SOURCE_DIR:-$default_entity_source}"
    extra_title_source="${EXTRA_TITLE_SOURCE_DIR:-$default_extra_title_source}"
    if [[ -z "${EXTRA_TITLE_SOURCE_DIR:-}" && ! -d "$extra_title_source/entities" ]]; then
      extra_title_source=""
    fi
    cmd=(
      "$PYTHON_BIN" "$BASE_DIR/prepare_seeded_from_test32.py"
      --artifact-source-dir "$artifact_source"
      --entity-source-dir "$entity_source"
      --dest-dir "$BASE_DIR"
      --target-persons "$N_PERSONS"
      --seed "$SEED"
    )
    if [[ -n "$extra_title_source" ]]; then
      cmd+=(--extra-title-source-dir "$extra_title_source")
    fi
    "${cmd[@]}"
    "$PYTHON_BIN" "$BASE_DIR/generate_character_bank.py" \
      --base-dir "$BASE_DIR" \
      --target "$N_CHARACTERS" \
      --seed "$SEED" \
      --mode research
    title_bank_path="$BASE_DIR/entities/title_bank.csv"
    title_rows=0
    if [[ -f "$title_bank_path" ]]; then
      title_rows=$(($(wc -l < "$title_bank_path") - 1))
    fi
    if (( title_rows < N_TITLES )); then
      "$PYTHON_BIN" "$BASE_DIR/topup_title_bank.py" \
        --base-dir "$BASE_DIR" \
        --target-count "$N_TITLES" \
        --seed "$SEED" \
        --start-year "$START_YEAR" \
        --end-year "$END_YEAR" \
        --mode research
    else
      echo "Title bank already has $title_rows rows; skipping top-up to $N_TITLES."
    fi
    run_preflight
    ;;
