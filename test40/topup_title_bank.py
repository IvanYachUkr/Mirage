"""Top up title_bank.csv for test30.
...
def _materialize_tagline_for_title(
    *,
    ...
    passes = [
        {"attempts": 48, "near_window": 24, "near_threshold": 0.93, "family_cap": _MAX_TAGLINE_TEMPLATE_FAMILY_REUSE},
        {"attempts": 96, "near_window": 12, "near_threshold": 0.975, "family_cap": max(_MAX_TAGLINE_TEMPLATE_FAMILY_REUSE + 1, 2)},
    ]
    best_candidate = ""
    best_family_sig = ""
    ...
    for cfg in passes:
        for _ in range(int(cfg["attempts"])):
            ...
            if family_count >= int(cfg["family_cap"]):
                continue
            if tagline_is_near_duplicate(candidate_tagline, tagline_history[-int(cfg["near_window"]):], threshold=float(cfg["near_threshold"])):
                continue
    ...
    for _ in range(96):
        ...
        if not (
            ...
            continue
        family_count = int(tagline_template_family_counts.get(template_family_sig, 0))
        near_penalty = int(tagline_is_near_duplicate(candidate_tagline, tagline_history[-24:], threshold=0.93))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Title bank top-up for test30")
