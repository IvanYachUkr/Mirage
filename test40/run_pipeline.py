...
def _check_title_bank(base_dir: Path, target_count: int, start_year: int | None = None, end_year: int | None = None) -> bool:
    ...
    if start_year is not None or end_year is not None:
        ...
        if years.isna().any():
            return False
        if not bool(years.between(int(start_year), int(end_year), inclusive="both").all()):
            return False
    return True
...
def _check_temporal_regime_plan(base_dir: Path) -> bool:
    return _check_json_artifact(temporal_regime_plan_path(base_dir), ("year_weights", "phases"))
...
def _resolve_step_model(args: argparse.Namespace, *, step_id: int, script: str) -> str | None:
    ...
    bulk_model = _clean_model_name(getattr(args, "bulk_artifact_model", None)) or str(
        MODEL_TIERS.get("artifact_bulk", "gemini-3.1-flash-lite-preview")
    )
...
def _build_steps(args: argparse.Namespace) -> list[dict]:
    return [
        ...
        },
        {
            ...
            "args": ["--base-dir", str(BASE_DIR), "--target", str(args.n_persons), "--seed", str(args.seed), "--mode", str(args.mode)],
            "check": lambda bd: _check_json_list_exact_count(bd / "entities" / "persons.json", int(args.n_persons)),
            "requires_api": False,
            "description": "Generate people from the research identity bank or the debug procedural fallback.",
        },
        {
            "id": 18,
        ...
        },
        {
            ...
            "args": ["--base-dir", str(BASE_DIR), "--target", str(args.n_companies), "--seed", str(args.seed), "--mode", str(args.mode)],
            "check": lambda bd: _check_json_list_exact_count(bd / "entities" / "companies.json", int(args.n_companies)),
            "requires_api": False,
            "description": "Generate companies from the research lexicon or the debug procedural fallback.",
        },
        {
            "id": 28,
        ...
        },
        {
            ...
            "args": ["--base-dir", str(BASE_DIR), "--target", str(args.n_keywords), "--seed", str(args.seed), "--mode", str(args.mode)],
            "check": lambda bd: _check_json_list_exact_count(bd / "entities" / "keywords.json", int(args.n_keywords)),
            "requires_api": False,
            "description": "Generate keywords from the research seed bank or the debug procedural fallback.",
        },
        {
            "id": 40,
        ...
        },
        {
            ...
            "args": ["--base-dir", str(BASE_DIR), "--target", str(args.n_characters), "--seed", str(args.seed), "--mode", str(args.mode)],
            "check": lambda bd: _check_csv_exact_count(bd / "entities" / "character_bank.csv", int(args.n_characters)),
            "requires_api": False,
            "description": "Generate character names from the research bank or the debug procedural fallback.",
        },
        {
            "id": 56,
            ...
            "check": _check_temporal_regime_plan,
            "requires_api": bool(args.mode == "research"),
            "enabled": bool(args.mode == "research"),
...
def main() -> None:
    ...
    parser.add_argument(
        "--bulk-artifact-model",
        default=str(MODEL_TIERS.get("artifact_bulk", "gemini-3.1-flash-lite-preview")),
    ...
    if args.resume_step100 and args.reset_step100_resume:
        raise ValueError("Use at most one of --resume-step100 and --reset-step100-resume")
