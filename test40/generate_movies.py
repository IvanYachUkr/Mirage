...
def assemble_movies(world: WorldState, n_movies: int = 5000,
                    ...
                    speed_audit = None,
                    resume_manager: Step100ResumeManager | None = None) -> dict:
    ...
    if resume_manager.is_resuming:
        ...
        global_tables["tv_series"] = pd.DataFrame(tv_data["tv_series"])
        global_tables["seasons"] = pd.DataFrame(tv_data["seasons"])
        global_tables["episodes"] = pd.DataFrame(tv_data["episodes"])
    ...
    print(f"  TV series: {len(global_tables.get('tv_series', pd.DataFrame()))} series, "
          ...
          f"{len(global_tables.get('episode_cast', pd.DataFrame()))} episode cast rows")
    ...
    if resume_manager.is_resuming:
        ...
        restored_state = resume_manager.restore_checkpoint(world, restore_graph=False)
        ...
        if planned_resume_start_index < len(plan_records):
            resume_manager.restore_graph(world)
...
def main(base_dir: str = None, n_movies: int = None,
    ...
    if resume_step100 and reset_step100_resume:
