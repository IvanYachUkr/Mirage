...
LOCAL_OLLAMA_PROFILE_ID="${LOCAL_OLLAMA_PROFILE_ID:-}"
...
detect_or_install_local_llm() {
  ...
  if [[ "$LAB_AUTO_INSTALL_OLLAMA" == "1" ]]; then
    if [[ -n "$LOCAL_OLLAMA_PROFILE_ID" ]]; then
      echo "No local endpoint detected; running Ollama installer profile $LOCAL_OLLAMA_PROFILE_ID."
    else
      echo "No local endpoint detected; running Ollama installer with hardware-based profile selection."
    fi
