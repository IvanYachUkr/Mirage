...
def print_qerror(preds_unnorm, labels_unnorm):
    ...
    for i in range(len(preds_unnorm)):
        if preds_unnorm[i] > labels_unnorm[i]:
            qerror.append(preds_unnorm[i] / labels_unnorm[i])
        else:
            qerror.append(labels_unnorm[i] / preds_unnorm[i])
...
def train_and_predict(workload_name, num_queries, num_epochs, batch_size, hid_units, cuda):
    ...
    preds_test_unnorm = np.asarray(preds_test_unnorm, dtype=float).reshape(-1)
