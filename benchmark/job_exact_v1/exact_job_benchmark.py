...
def evaluate_state(
    *,
    ...
    try:
        has_rows = runner.scalar_int(strip_to_exists(sql) + ";")
        if has_rows:
...
def evaluate_relaxed_support(
    *,
    ...
    try:
        actual = runner.scalar_int(strip_to_exists(sql) + ";")
        result = (actual, None)
    except Exception as exc:
        result = (0, str(exc).splitlines()[0][:240])
...
def beam_search_query(
    *,
    ...
    witness_states = mine_joint_witness_states(
        base_sql=original_sql,
        slots=slots,
        runner=runner,
    ...
    if best_error:
        ...
        try:
            relaxed_count = runner.scalar_int(strip_to_exists(relaxed_all_sql) + ";")
        except Exception:
            relaxed_count = 0
        if relaxed_count == 0:
