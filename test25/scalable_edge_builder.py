...
STAGE_PRIORITY = {"legend": 0, "veteran": 1, "prime": 2, "retired": 3, "rising": 4}
TIER_ORDER = {"Global": 0, "Major": 1, "Mid-Budget": 2, "Indie": 3, "Micro": 4}
COLLAB_STYLE_CODES = {"ensemble": 0, "selective": 1, "auteur": 2}
...
def _person_stochastic_cap(pid: int, stage: str) -> int:
    ...
    mu, sigma, lo, hi = params.get(stage, (10, 5, 1, 30))
...
def _build_person_features(persons: list[dict[str, Any]], latent_map: Mapping[int, Mapping[str, Any]]) -> PersonFeatures:
    ...
    for idx, person in enumerate(persons):
        ...
        debut[idx] = _safe_year(person.get("debut_year"), YEAR_LO + (pid % 25))
        retire[idx] = max(debut[idx], _safe_year(person.get("retirement_year"), YEAR_HI + 10))
        ...
        collaboration_style[idx] = int(COLLAB_STYLE_CODES.get(collab_style, 0))
...
def _build_graph_profile(persons: PersonFeatures, companies: CompanyFeatures, pools: HotGraphPools) -> GraphBuildProfile:
    ...
    ensemble_share = float(np.mean(persons.collaboration_style == COLLAB_STYLE_CODES["ensemble"])) if len(persons.collaboration_style) else 0.0
    ...
    friendship_ratio = _clamp(0.16 + 0.10 * creative_share + 0.18 * community_density, 0.18, 0.34)
    rivalry_ratio = _clamp(0.045 + 0.12 * competition_density + 0.05 * actor_share, 0.05, 0.16)
    rivalry_actor_coverage = _clamp(0.38 + 0.45 * competition_density + 0.08 * controversy_tail, 0.45, 0.85)
    mentorship_ratio = _clamp(0.05 + 0.18 * (director_n / creative_n) + 0.05 * (average_cap / 20.0), 0.06, 0.16)
    avoid_ratio = _clamp(0.01 + 0.10 * controversy_tail + 0.03 * competition_density, 0.012, 0.07)
    former_ratio = _clamp(0.04 + 0.12 * community_density + 0.05 * creative_share, 0.05, 0.16)
    collaboration_ratio = _clamp(0.035 + 0.08 * ensemble_share + 0.06 * community_density, 0.04, 0.14)
    clique_ratio = _clamp(0.022 + 0.12 * community_density, 0.025, 0.09)
    closure_scale = _clamp(0.70 + 0.80 * community_density, 0.70, 1.50)
    bridge_anchor_ratio = _clamp(0.015 + 0.030 * (community_count / max(1.0, np.sqrt(float(people_n)))), 0.015, 0.060)

    brand_base = _clamp(0.18 + 0.65 * companies_per_person, 0.20, 0.90)
    employment_base = _clamp(0.009 + 0.060 * companies_per_person, 0.012, 0.060)
    ...
    return GraphBuildProfile(
        ...
        friendship_threshold=_clamp(0.34 + 0.06 * (1.0 - community_density), 0.30, 0.46),
        rivalry_threshold=_clamp(0.30 + 0.08 * (1.0 - competition_density), 0.28, 0.44),
        mentorship_threshold=_clamp(0.34 + 0.04 * (1.0 - community_density), 0.30, 0.42),
        avoid_threshold=_clamp(0.40 - 0.10 * controversy_tail, 0.30, 0.42),
        former_threshold=_clamp(0.30 + 0.08 * (1.0 - community_density), 0.26, 0.40),
        collaboration_threshold=_clamp(0.25 + 0.08 * (1.0 - ensemble_share), 0.22, 0.38),
        clique_threshold=_clamp(0.34 + 0.06 * (1.0 - community_density), 0.30, 0.44),
        company_genre_sample_ratio=_clamp(1.9 + 2.0 * companies_per_person, 1.8, 6.0),
        company_clique_sample_ratio=_clamp(0.8 + 1.2 * clique_density, 0.8, 3.0),
        company_random_sample_ratio=_clamp(0.7 + 1.0 * companies_per_person, 0.7, 2.5),
        brand_fit_ratio_by_stage=brand_fit_ratio_by_stage,
        employment_ratio_by_stage=employment_ratio_by_stage,
        brand_fit_threshold=_clamp(0.26 + 0.08 * (1.0 - companies_per_person * 10.0), 0.20, 0.34),
        blacklist_threshold=_clamp(0.42 - 0.08 * controversy_tail, 0.30, 0.42),
        cc_genre_sample_ratio=_clamp(2.4 + 2.6 * clique_density, 2.0, 6.0),
        cc_clique_sample_ratio=_clamp(1.4 + 1.4 * clique_density, 1.4, 4.0),
        cc_market_sample_ratio=_clamp(1.4 + 1.8 * clique_density, 1.4, 4.5),
        cc_random_sample_ratio=_clamp(0.8 + 0.4 * clique_density, 0.8, 2.0),
        cc_rival_pick_ratio=_clamp(0.020 + 0.045 * clique_density, 0.018, 0.060),
        cc_copro_pick_ratio=_clamp(0.014 + 0.040 * clique_density, 0.014, 0.050),
        cc_subsidiary_pick_ratio=_clamp(0.06 + 0.10 * clique_density, 0.05, 0.20),
    )
...
def _build_friendship_edges(
    ...
    order = np.argsort(
        np.array(
            [STAGE_PRIORITY.get(str(stage), 2) * 1000 - int(cap) for stage, cap in zip(persons.stage_name, persons.cap)],
            dtype=np.int32,
        ),
        kind="mergesort",
    )
        ...
        target = int(
            round(
                ...
                * (
                    ...
                    + 0.04 * float(persons.collaboration_style[idx] == COLLAB_STYLE_CODES["ensemble"])
...
def _yield_person_company_batches(
    ...
    *,
    ...
    valid_from_years = np.maximum(YEAR_LO, persons.debut.astype(np.int32, copy=False))
    valid_to_years = np.maximum(persons.debut.astype(np.int32, copy=False), np.minimum(persons.retire.astype(np.int32, copy=False), YEAR_HI + 10))
...
def _yield_company_company_batches(
    ...
    *,
    ...
    for idx in range(len(companies.ids)):
        ...
        rival_score = 0.40 * genre_overlap + 0.24 * tier_sim + 0.16 * same_market + 0.12 * (1.0 - same_clique * 0.6) + 0.08 * risk_match
        ...
        if rival_pick.size:
            ...
            buffers["market_rival"]["valid_from"].append(np.full(count, YEAR_LO, dtype=np.int32))
            buffers["market_rival"]["valid_to"].append(np.full(count, YEAR_HI + 10, dtype=np.int32))
            queued_counts["market_rival"] += count

        complementary_tier = 1.0 - np.abs(tier_gap - 1.0) / 4.0
        cross_market_bonus = 1.0 - same_market * 0.5
        copro_score = 0.42 * genre_overlap + 0.18 * complementary_tier + 0.16 * cross_market_bonus + 0.14 * (1.0 - same_clique * 0.4) + 0.10 * risk_match
        ...
        if copro_pick.size:
            ...
            buffers["co_production"]["valid_from"].append(np.full(count, YEAR_LO, dtype=np.int32))
            buffers["co_production"]["valid_to"].append(np.full(count, YEAR_HI + 10, dtype=np.int32))
    ...
    for clique_id, members in companies.clique_members.items():
        ...
        buffers["subsidiary"]["valid_from"].append(np.full(len(children), YEAR_LO, dtype=np.int32))
        buffers["subsidiary"]["valid_to"].append(np.full(len(children), YEAR_HI + 10, dtype=np.int32))
...
def build_scalable_runtime_graph(
    ...
    *,
    ...
    def _iter_batches() -> Iterator[dict[str, Any]]:
        ...
        yield from _yield_person_company_batches(person_features, company_features, profile, rng, stats=cp_stats)
        ...
        yield from _yield_company_company_batches(company_features, profile, rng, stats=cc_stats)
