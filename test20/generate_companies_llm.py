...
def call_api_with_timeout(llm, model, prompt, config, timeout=70):
...
def call_with_retry(llm, model, prompt, config, max_retries=5, timeout=70):
    """Call LLM with retry on 503/timeout. Returns (resp, inp, out, cost)."""
    for retry in range(max_retries):
        try:
            resp = call_api_with_timeout(llm, model, prompt, config, timeout=timeout)
