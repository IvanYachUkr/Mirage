...
def call_with_timeout(llm, model, contents, config, timeout):
    temperature = config.get("temperature", 0.7)
    max_tokens = config.get("max_output_tokens", 8192)
    return llm.generate(
        contents if "contents" in dir() else prompt,
...
def call_with_retry(llm, model, contents, config, timeout, max_retries=5):
    for attempt in range(max_retries):
        try:
            return call_with_timeout(llm, model, contents, config, timeout)
        except TimeoutError:
...
def main():
    ...
    for b_idx, start in enumerate(range(0, len(todo), args.batch_size)):
        ...
        try:
            resp = call_with_retry(llm, args.model, prompt, config,
