...
def call_with_timeout(llm, prompt, config, timeout=70):
    """LLM call with timeout — delegates to llm.generate()."""
...
def call_api(llm, prompt, config, max_retries=5, timeout=70):
    """Call API with 503 detection + exponential backoff. Returns (parsed_list, cost)."""
    for retry in range(max_retries):
        try:
            resp = call_with_timeout(llm, prompt, config, timeout=timeout)
...
def step1_generate(llm, num_cliques, force=False):
    print("=" * 60)
...
def step2_assign(llm, cliques, batch_size=80, timeout=70):
    print("\n" + "=" * 60)
...
def main():
    ...
    if args.step in (None, 1):
        cliques = step1_generate(llm, args.num_cliques, force=args.force)
    else:
    ...
    if args.step in (None, 2):
        step2_assign(llm, cliques, batch_size=args.batch_size, timeout=args.timeout)
