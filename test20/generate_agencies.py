...
def call_api(llm, prompt, config, max_retries=5):
    """Call LLM with retry. Returns (parsed_list, cost)."""
    ...
    for retry in range(max_retries):
        try:
            ...
            label = "503 " if is_503 else ""
            print(f"  {label}API ERROR (retry {retry+1}): {e}")
            time.sleep((70 if is_503 else 5) * (retry + 1))
            continue

        print(f"  {resp.input_tokens:,}in+{resp.output_tokens:,}out=${resp.cost_usd:.4f}")
...
def step1_generate(llm, force=False):
    print("=" * 60)
...
def step2_assign(llm, agencies, batch_size=150):
    print("\n" + "=" * 60)
...
def main():
    ...
    if args.step in (None, 1):
        agencies = step1_generate(llm, force=args.force)
    else:
    ...
    if args.step in (None, 2):
        step2_assign(llm, agencies, batch_size=args.batch_size)
