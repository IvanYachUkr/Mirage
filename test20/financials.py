...
def _graph_synergy(world, year: int, movie_id: int, cast_ids: List[int], director_id: Optional[int]) -> float:
    ...
    for pid in cast_set:
        for (nbr, w, vf, vt) in friend_adj.get(int(pid), []):
    ...
    for pid in cast_set:
        for (nbr, w, vf, vt) in rival_adj.get(int(pid), []):
            if nbr not in cast_set:
                continue
            key = (min(pid, nbr), max(pid, nbr))
            if key in pairs_seen:
    ...
    aff = getattr(world, "affinity_index", None) or {}
    if director_id:
        for pref in aff.get("director_prefs", {}).get(int(director_id), []):
            if isinstance(pref, Mapping):
                pref_id = pref.get("actor_id")
                pref_w = pref.get("weight", 0.8)
                pref_vf = pref.get("valid_from")
                pref_vt = pref.get("valid_to")
            elif isinstance(pref, (list, tuple)):
                pref_id = pref[0]
                pref_w = pref[1] if len(pref) > 1 else 0.8
                pref_vf = pref_vt = None
            else:
                pref_id = pref
                pref_w = 0.8
                pref_vf = pref_vt = None
        ...
        for av in aff.get("director_avoids", {}).get(int(director_id), []):
            if isinstance(av, Mapping):
                av_id = av.get("actor_id", -1)
                av_vf = av.get("valid_from")
                av_vt = av.get("valid_to")
            elif isinstance(av, int):
                av_id = av
                av_vf = av_vt = None
            else:
                av_id = av[0] if av else -1
                av_vf = av_vt = None
