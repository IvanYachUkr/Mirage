...
class WorldState:
    ...
    def _load_edge_graph(self):
        ...
        if arrow_path.exists():
            try:
                ...
                print(f"  WARNING: failed to read edge_graph.arrow ({exc}), falling back to CSV")
                df = None
        ...
        eg = EdgeGraph()
        # Build id_to_name before freeing the DataFrame
        ...
        # Convert to list[dict] for runtime compatibility, then free DataFrame
        ...
        print(f"  Loaded {len(eg.edges):,} edges from {source}")
    ...
    def _build_pc_scoring_arrays(self):
        ...
        for i, pid in enumerate(p_pids):
            ...
            csv_raw = lv.get("creative_style_vector", [])
            if isinstance(csv_raw, list):
                for j in range(min(12, len(csv_raw))):
                    self._pc_p_genre[i, j] = float(csv_raw[j])
        ...
        for i, cid in enumerate(c_cids):
            ...
            gp = lv.get("genre_portfolio", [0.083] * 12)
            if isinstance(gp, list):
                for j in range(min(12, len(gp))):
                    self._pc_c_genre[i, j] = float(gp[j])
    ...
    def compute_pc_affinity_batch(self, pids: np.ndarray, cid_set: set) -> np.ndarray:
        ...
        c_idxs = np.array([self._pc_c_idx[cid] for cid in cid_set
                           if cid in self._pc_c_idx], dtype=int)
        if len(c_idxs) == 0:
            return np.ones(len(pids), dtype=float)

        # Company arrays for candidates
        c_risk = self._pc_c_risk[c_idxs]                  # (M,)
        c_budget = self._pc_c_budget[c_idxs]               # (M, 5)
        c_genre = self._pc_c_genre[c_idxs]                 # (M, 12)
        ...
        for i, pid in enumerate(pids):
            ...
            if p_idx is None:
                continue
            # Vectorized scoring: this person vs all candidate companies
            risk_match = 1.0 - np.abs(self._pc_p_risk[p_idx] - c_risk)        # (M,)
            budget_overlap = np.sum(
                self._pc_p_budget[p_idx] * c_budget, axis=1)                   # (M,)
            genre_overlap = np.sum(
                self._pc_p_genre[p_idx, :c_genre.shape[1]] * c_genre, axis=1)  # (M,)
            raw = 0.30 * risk_match + 0.30 * budget_overlap + 0.30 * genre_overlap
            # Brand-fit threshold: 0.60 (matches generate_edges_hybrid)
            if np.any(raw > 0.60):
                out[i] = 1.8  # same boost as old _person_company_multiplier
        return out
    ...
    def compute_cc_affinity(self, cid_a: int, cid_b: int) -> float:
        ...
        return 0.40 * genre_sim + 0.35 * tier_sim + 0.25 * risk_match
