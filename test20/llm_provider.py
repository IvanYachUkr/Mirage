...
Supports two backends:
  ...
  2. **OpenAICompatibleProvider** — vLLM, Ollama, TGI, or any OpenAI-compatible server
...
Configuration via environment / .env::
    ...
    LOCAL_LLM_MODEL=nvidia/Nemotron-3-Super-120B
...
def _call_with_timeout(fn: Callable[[], Any], timeout_sec: float) -> Any:
    ...
    try:
        ...
        try:
            ...
            pool.shutdown(wait=False)
        raise TimeoutError(f"LLM call exceeded {timeout_sec:.0f}s") from exc
...
class GeminiProvider(LLMProvider):
    ...
    def __init__(self, api_key: str, default_model: str = "gemini-3.1-flash-lite-preview"):
...
class OpenAICompatibleProvider(LLMProvider):
    ...
    Works with:
      - vLLM: ``python -m vllm.entrypoints.openai.api_server --model nvidia/Nemotron-3-Super-120B``
    ...
    def __init__(
        ...
        model: str = "nvidia/Nemotron-3-Super-120B",
        ...
        try:
            ...
            raise ImportError(
                "The 'requests' package is required for local LLM provider. "
...
def get_llm_client(
    *,
    ...
    if provider_name in ("gemini", "google"):
        ...
        # Get default model from contracts if available
        default_model = "gemini-3.1-flash-lite-preview"
        try:
            from contracts import MODEL_TIERS
            default_model = MODEL_TIERS.get("entity_gen", default_model)
        except ImportError:
            pass
        ...
        model = os.getenv("LOCAL_LLM_MODEL", "nvidia/Nemotron-3-Super-120B")
