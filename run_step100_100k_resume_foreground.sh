...
export DATA_SYS_NEAR_DUPLICATE_LIMIT="${DATA_SYS_NEAR_DUPLICATE_LIMIT:-50000}"

LOG_DIR="$BASE_DIR/_runner_logs/candidate100k_2000_2050"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/foreground_step100_100k_resume.log"
PID_FILE="$LOG_DIR/foreground_step100_100k_resume.pid"

printf "%s\n" "$$" > "$PID_FILE"
{
  printf "started foreground %s\n" "$$"
  printf "log %s\n" "$LOG_FILE"
  "$BASE_DIR/.venv-linux/bin/python" "$BASE_DIR/run_pipeline.py" \
    --n-movies "$N_MOVIES" \
