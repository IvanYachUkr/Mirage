...
def apply_world_patches(
    ...
    edge_graph = getattr(world, "edge_graph", None)
    edges = edge_graph.edges if edge_graph is not None else None
    edge_lookup = _build_edge_lookup(edges) if isinstance(edges, list) else None
    ...
    for op in patches:
        try:
            ...
            if kind in {"edge_add", "edge_update", "edge_expire"}:
                if edges is None:
                    rep.skipped += 1
                    rep.log(f"{kind}: world has no edge_graph")
                    continue
                ...
                if not src_ok or not dst_ok:
                    ...
                    continue
                idx = _find_edge_index(edges, src, dst, edge_type, lookup=edge_lookup)

                if kind == "edge_add":
                    if idx is not None and _edge_is_active(edges[idx]):
                        rep.skipped += 1
                    ...
                    edges.append(new_edge)
                    if edge_lookup is not None:
                        edge_lookup[_edge_key(src, dst, edge_type)] = (len(edges) - 1, float(weight))
                ...
                if kind == "edge_update":
                    new_weight = float(edges[idx].get("weight", 0.0) or 0.0)
                    if "weight" in op:
                    ...
                    _version_edge(edges, idx, from_year, edge_lookup=edge_lookup, weight=float(new_weight), reason=_safe_str(op.get("reason"), edges[idx].get("reason", "")))
                ...
                year = max(from_year, min(to_year, _safe_int(op.get("year"), to_year)))
                vf = _safe_int(edges[idx].get("valid_from"), from_year)
                edges[idx]["valid_to"] = int(max(vf, year))
                edges[idx]["_scd2_retired"] = True
                expired_ops.append((src, dst, edge_type))
                rep.applied += 1
                continue
    ...
    if expired_ops or add_or_update_ops:
        ...
        # Invalidate _ensure_edge_arrays cache: edge_expire modifies
        # _scd2_retired/valid_to in-place without appending, so the
        # length-based staleness check alone is insufficient.
...
def _emit_edge_program_patches(
    ...
    *coherent* changes driven by the current activity graph and the yearly regime.
    ...
    if edge_graph is not None and getattr(edge_graph, "edges", None):
        n_edges = len(edge_graph.edges)
        # Target: ~1.5% of edges, capped at 200k to avoid memory issues
        sample_size = max(400, min(200_000, n_edges // 60 + len(ctx.active_person_ids)))
        _ensure_edge_arrays(edge_graph)  # Build/refresh vectorized arrays
        for idx in _background_edge_sample(edge_graph.edges, from_year, sample_size, seed=_stable_hash(f"edge_sample|{from_year}|{to_year}"), edge_graph=edge_graph):
            edge = edge_graph.edges[idx]
...
def _amplify_genre_cascade(
    world, genre: str, delta: float, from_year: int, rng: random.Random,
) -> List[Dict[str, Any]]:
    ...
    if delta > 0.02 and len(affected_pids) >= 4:
        ...
        for i in range(min(40, len(sample) - 1)):
...
def _amplify_regional_wave(
    world, ctx: ExposureContext, from_year: int, rng: random.Random,
) -> List[Dict[str, Any]]:
    ...
    for country, share in hot_countries:
        ...
        if len(country_pids) >= 4:
            ...
            for i in range(min(30, len(sample) - 1)):
                a, b = sample[i], sample[(i + 1) % len(sample)]
                if a == b:
                    continue
                patches.append({
...
def procedural_year_step(world, from_year: int, to_year: int, year_bucket: List[Dict[str, Any]]) -> PatchApplyReport:
    ...
    # P5-FIX: mark adjacency dirty for lazy rebuild instead of immediate O(N) rebuild.
    if apply_rep.applied > 0 and hasattr(world, '_mark_adjacency_dirty'):
...
def _edges_for_entities(world, people: Sequence[int], companies: Sequence[int], year: int, limit: int = 260) -> List[Dict[str, Any]]:
    """Collect edges whose both endpoints are in the given people/company sets.

    B3-FIX: uses pre-built adjacency lists (``_friend_adj_all``, ``_rival_adj_all``)
    for O(sum-of-degrees) traversal instead of scanning all 7M edges.
    Falls back to full scan only when adjacency lists are unavailable.
    """
    edge_graph = getattr(world, "edge_graph", None)
    if edge_graph is None:
        return []
    pset = set(int(x) for x in people)
    cset = set(int(x) for x in companies)

    # ---- Fast path: traverse adjacency lists (O(sum of degrees)) ----
    # P5-FIX: trigger lazy rebuild if adjacency was dirtied since last access
    if hasattr(world, '_ensure_edge_adjacency'):
        world._ensure_edge_adjacency()
    friend_adj = getattr(world, "_friend_adj_all", None)
    rival_adj = getattr(world, "_rival_adj_all", None)
    if friend_adj is not None or rival_adj is not None:
        rows: List[Dict[str, Any]] = []
        seen: set[tuple] = set()

        def _collect_from_adj(adj: Dict[int, list], edge_type: str, sign: str) -> None:
            if adj is None:
                return
            for pid in pset:
                for (nbr, w, vf, vt) in adj.get(pid, []):
                    if nbr not in pset:
                        continue
                    key = (min(pid, nbr), max(pid, nbr), edge_type)
                    if key in seen:
                        continue
                    seen.add(key)
                    # Validity window check
                    if vf is not None and int(vf) > year:
                        continue
                    if vt is not None and int(vt) < year:
                        continue
                    rows.append({
                        "src_id": key[0],
                        "dst_id": key[1],
        ...
        _collect_from_adj(friend_adj, "friendship", "+")
        if len(rows) < limit:
            _collect_from_adj(rival_adj, "rivalry", "-")
        return rows[:limit]

    # ---- Slow fallback: full edge scan (O(N_edges)) ----
    rows = []
    for edge in edge_graph.edges:
        if not _edge_is_active(edge, year):
            continue
        edge_type = _safe_str(edge.get("edge_type"), "")
        src = _safe_int(edge.get("src_id"))
        dst = _safe_int(edge.get("dst_id"))
        src_type, dst_type = _EDGE_ENTITY_TYPES.get(edge_type, ("person", "person"))
        src_ok = src in (cset if src_type == "company" else pset)
        dst_ok = dst in (cset if dst_type == "company" else pset)
...
def llm_year_step(
    ...
    if genai is None:
        rep.skipped += 1
        rep.log("google-genai not installed; skipping LLM evolution")
        return rep

    api_key = os.getenv(api_key_env) or os.getenv("GOOGLE_API_KEY")
    if not api_key:
        rep.skipped += 1
        rep.log(f"Missing {api_key_env} / GOOGLE_API_KEY; skipping LLM evolution")
        return rep

    if model is None:
        try:
            from contracts import MODEL_TIERS
            model = MODEL_TIERS.get("temporal_evolution", "gemini-2.5-flash")
        except ImportError:
            model = os.getenv("TEMPORAL_EVOLUTION_MODEL", "gemini-2.5-flash")
    ...
    # P5-FIX: mark adjacency dirty for lazy rebuild instead of immediate O(N) rebuild.
    if rep.applied > 0 and hasattr(world, '_mark_adjacency_dirty'):
        world._mark_adjacency_dirty()
