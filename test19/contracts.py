"""
Test 11 -- Phase 0: Contracts, Schemas, Vocabularies, Validators
===============================================================
...
RELATIONSHIP_TARGETS = {
    # Lower rates: calibration should only TOP UP genuinely underrepresented
    # relationship types, NOT force-connect naturally isolated rising actors.
    # Rising actors with deg=0 stay at deg=0 -- they earn connections via assembly.
    "best_friend_rate": 0.15,       # 15% of actors get a calibrated best-friend edge
    "rival_rate": 0.08,             # 8% get a calibrated rivalry
    "bf_same_community_rate": 0.60, # 60% of BF pairs should be same community (unchanged)
    "director_preferred_actors": 3,  # each director prefers ~3 actors (was 5)
    "director_avoided_actors": 1,    # each director avoids ~1 actor (was 2)
}
...
def generate_compositional_title(rng, used: set) -> str:
    ...
    for _ in range(100):
        ...
        if title not in used:
            return title
    # Absolute fallback: append number
...
SNAPSHOT_CONFIG = {
    "snapshot_id": "test19_v1",  # C3-FIX: was stale "test11_v1"
    "generator_version": "2.3.0",
    "seed": 42,
    "write_yearly_snapshots": True,  # V17: ON by default -- never lose temporal data
}

# v11: Tiered model strategy -- different models per pipeline step.
# Flash Lite: bulk structured data (JSON numbers/names). Cheap, fast, sufficient.
# Flash:      reasoning + graph evolution. Needs more coherence than Lite.
# Pro:        creative text (plot summaries, descriptions). Best prose quality.
# Override any entry via the --model CLI arg on individual scripts.
MODEL_TIERS = {
    "latent_vars":       "gemini-3.1-flash-lite-preview",
    "entity_gen":        "gemini-3.1-flash-lite-preview",
    "edge_generation":   "gemini-3.1-flash-lite-preview",
    "temporal_evolution": "gemini-3.1-flash-lite-preview",
    "plot_summaries":    "gemini-3.1-flash-lite-preview",
    "character_descriptions": "gemini-3.1-flash-lite-preview",
}
