...
def _person_company_multiplier(world: WorldState, pids: np.ndarray, tier: str, genre: str) -> np.ndarray:
    aff = (world.affinity_index or {}).get("person_company_affinity", {})
    if not aff:
    ...
    for i, pid in enumerate(pids):
        for edge in aff.get(int(pid), []):
            if int(edge.get("company_id", -1)) in suitable:
                out[i] = 1.8
                break
    return out
...
def pick_director(world: WorldState, concept: dict) -> int | None:
    ...
    pc_aff = (world.affinity_index or {}).get("person_company_affinity", {})
    if suitable and pc_aff:
        for i, pid in enumerate(dir_pids):
            for edge in pc_aff.get(int(pid), []):
                if int(edge.get("company_id", -1)) in suitable:
                    weights[i] *= 2.0
                    break
...
def pick_companies(world: WorldState, concept: dict, director_id: int) -> list[dict]:
    ...
    if n_companies > 1:
        aff = world.affinity_index or {}
        co_aff = aff.get("company_affinity", {})
        co_riv = aff.get("company_rivalry", {})
        ...
        for i, cid in enumerate(cids):
            key = (min(first_cid, int(cid)), max(first_cid, int(cid)))
            if key in co_aff:
                partner_weights[i] *= (1.0 + 3.0 * _edge_payload_weight(co_aff[key]))
            if key in co_riv:
                partner_weights[i] *= max(0.05, 1.0 - _edge_payload_weight(co_riv[key]))
...
def pick_cast(world: WorldState, concept: dict, director_id: int, max_retries: int = 3) -> tuple[list[dict], list[tuple[int, int]]]:
    ...
    for _attempt in range(max_retries):
        ...
        for slot in range(cast_size):
            ...
            if slot >= 2:
                scores[static.unused_flags] *= 6.0
            if slot >= 4:
