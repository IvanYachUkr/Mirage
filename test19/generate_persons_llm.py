...
def _call_api_with_timeout(client, model, prompt, config, timeout=70):
    """Call Gemini API with a hard timeout. Raises TimeoutError if stuck > timeout seconds."""
    ...
    try:
        ...
        pool.shutdown(wait=False, cancel_futures=True)
        raise TimeoutError(f"API call exceeded {timeout}s hang limit")
