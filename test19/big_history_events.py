...
def _sample_people(world, year: int, limit: int = 0) -> List[Dict[str, Any]]:
    ...
    if {"debut_year", "retirement_year"}.issubset(df.columns):
        active = df[
            (df["debut_year"].fillna(1970).astype(int) <= int(year))
            & (df["retirement_year"].fillna(2100).astype(float) >= float(year))
        ]
        if len(active) == 0:
            active = df
...
def _resolve_client(llm_client=None):
...
def generate_and_apply_events(
    ...
    if model is None:
        model = str(MODEL_TIERS.get("big_history_events") or MODEL_TIERS.get("temporal_evolution") or "gemini-2.5-flash")
    ...
    try:
        raw_text, parsed = _call_llm(client, str(model), prompt)
