...
def _call_api_with_timeout(client, model, prompt, config, timeout=API_TIMEOUT):
    """Single-attempt timeout-guarded API call."""
...
class TokenTracker:
    ...
    def record(self, response):
        """Extract and record token usage from a response BEFORE parsing.
        
        This is called immediately after the API response, before any 
        JSON parsing that might raise, so we never lose token metadata.
        """
        ...
        cost = inp / 1e6 * 0.25 + out / 1e6 * 1.50  # gemini-3-flash-preview: $0.25/$1.50 per 1M
        ...
        if persons_needing:
            ...
            for batch_start in range(0, len(persons_needing), _batch_size):
                ...
                for retry in range(MAX_RETRIES):
                    t0 = time.time()
                    try:
                        response = _call_api_with_timeout(
                            client, _model, prompt,
