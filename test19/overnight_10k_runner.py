...
def stage_05_llm_person_fill(ctx: Ctx) -> dict[str, Any]:
    ...
    for s in range(0, len(todo), 60):
        ...
        try:
            resp, st = generate_content_resilient(
                client=client,
                model=str(ctx.args.llm_model),
                contents=prompt,
                config={"temperature": 0.7, "max_output_tokens": len(batch) * 260, "thinking_config": {"thinking_budget": 0}},
                timeout_sec=70,
                max_attempts=5,
                base_delay_sec=4,
                max_delay_sec=30,
            ...
            timeout_r += int(st.timeout_retries)
            service_r += int(st.service_retries)

            i, o, c = usage_tokens_and_cost(
                resp,
                input_per_million=float(ctx.args.price_per_million),
                output_per_million=float(ctx.args.price_per_million),
            )
            tin += i
            tout += o
            cost += c
