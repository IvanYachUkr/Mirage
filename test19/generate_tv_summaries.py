...
class TokenTracker:
    ...
    def record(self, model: str, response) -> tuple[int, int, float]:
        ...
        rates = COST_RATE.get(model, {"in": 0.10, "out": 0.40})
        cost = inp / 1e6 * rates["in"] + out / 1e6 * rates["out"]
...
def load_data(base_dir: Path) -> dict:
    """Load all TV-related CSVs into DataFrames."""
    def _load(name):
        p = base_dir / f"{name}.csv"
        if not p.exists():
            print(f"  WARNING: {name}.csv not found")
            return pd.DataFrame()
        return pd.read_csv(p)

    series_df = _load("tv_series")
    seasons_df = _load("seasons")
    episodes_df = _load("episodes")
    ep_cast_df = _load("episode_cast")
...
def call_api(client, model: str, prompt: str, tracker: TokenTracker,
            ...
            def _do_call():
                try:
                    _result[0] = client.models.generate_content(
                        model=model,
                        contents=prompt,
                        config={"temperature": 0.75, "max_output_tokens": 8192},
                    )
...
def main():
    ...
    if not args.dry_run:
        if not api_key:
            raise RuntimeError(
                ...
                "Or set: $env:GEMINI_API_KEY='your-key'"
            )
    ...
    if series_df.empty:
        print("tv_series.csv not found or empty -- run generate_movies.py first")
        sys.exit(1)
    ...
    for i, series_row in enumerate(all_series):
        ...
        text = call_api(client, t1_model, prompt, tracker,
    ...
    for i, series_row in enumerate(all_series):
        ...
        for sn_idx, season in enumerate(seasons):
            ...
            text = call_api(client, t2_model, prompt, tracker,
