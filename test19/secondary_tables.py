...
def generate_user_ratings(movie_rows: list[dict], rng,
    ...
    for movie in movies:
        ...
        # Accumulate into flat arrays (avoid per-row dict construction)
        ...
        # Format dates as strings in a batch
        all_dates.extend(
...
def generate_production_timeline(movies: list, rng) -> list:
...
def generate_streaming_windows(movies: list, rng) -> list:
    ...
    for m in movies:
        ...
        for i, platform in enumerate(chosen):
            ...
            rows.append({"movie_id": mid, "platform": str(platform), "window_start": ws, "window_end": we, "exclusivity": int(i == 0 and rng.random() < 0.40)})
    return rows
...
def generate_person_contracts(persons_df, companies_df, cast_info: list, rng) -> list:
    """Generate talent contracts with salary bands and validity windows.

    V17-PERF: Replaced iterrows() with extracted arrays for ~10x speedup.
    Random call pattern preserved per-person for identical output.
    """
    ...
    # V17-PERF: Extract arrays once instead of iterrows (avoids Series overhead)
    ...
    _stages = persons_df["career_stage"].astype(str).values if "career_stage" in persons_df.columns else np.full(len(persons_df), "rising")
    rows = []
    for idx in range(len(_pids)):
        pid = int(_pids[idx])
        debut = int(_debuts[idx])
        retire = int(_retires[idx])
        stage = str(_stages[idx])
        n = int(rng.choice([1, 1, 2, 2, 3], p=[0.20, 0.30, 0.30, 0.15, 0.05]))
        start = debut
        for _ in range(n):
            if start >= retire:
                break
            cid = int(rng.choice(company_ids))
            dur = int(rng.choice([1, 2, 2, 3, 3, 4], p=[0.15, 0.30, 0.25, 0.15, 0.10, 0.05]))
            end = min(start + dur, retire)
            rows.append({
                "person_id": pid, "company_id": cid,
                "start_date": f"{start:04d}-01-01", "end_date": f"{end:04d}-12-31",
                "salary_band": str(rng.choice(_BANDS.get(stage, ["500K-1M"]))),
                "contract_type": str(rng.choice(_TYPES.get(stage, ["non_exclusive"]))),
            })
            start = end + int(rng.choice([0, 1, 2], p=[0.30, 0.50, 0.20]))
    return rows
...
def generate_movie_sequence(movies: list) -> list:
    """Materialise franchise ordering with predecessor links."""
    by_franchise = {}
    for m in movies:
        fid = m.get("franchise_id")
        if fid is None:
            continue
        by_franchise.setdefault(fid, []).append((int(m.get("installment_no") or 0), int(m.get("title_id") or 0)))
    rows = []
    for fid, lst in by_franchise.items():
        lst.sort()
        for i, (inst, mid) in enumerate(lst):
            rows.append({"franchise_id": fid, "movie_id": mid, "sequence_no": inst, "predecessor_movie_id": lst[i - 1][1] if i > 0 else None})
    return rows
...
def generate_person_collaborations(cast_info: list, movies: list) -> list:
    ...
    P4-FIX: caps per-movie cast to top 20 for pair generation, and
    limits total pairs to 500k to prevent memory exhaustion at scale.
    """
    if not cast_info:
        return []
    from itertools import combinations
    from collections import defaultdict
    movie_meta = {int(m.get("title_id", 0)): m for m in movies}
    ...
    MAX_TOTAL_PAIRS = 500_000  # P4-FIX: hard memory cap

    pairs: dict[tuple[int, int], list[int]] = defaultdict(list)
    total_pair_entries = 0
    for mid, pids in movie_persons.items():
        # P4-FIX: only top-billed actors generate collaboration pairs
        capped = pids[:MAX_CAST_FOR_PAIRS]
        for pa, pb in combinations(sorted(capped), 2):
            pairs[(pa, pb)].append(mid)
            total_pair_entries += 1
        if total_pair_entries > MAX_TOTAL_PAIRS:
            break  # P4-FIX: stop accumulating if memory cap hit

    rows = []
    for (pa, pb), mids in pairs.items():
        if len(mids) < 2:
            continue
        yrs = [int(movie_meta[m].get("year", 0)) for m in mids if m in movie_meta]
        genres = [str(movie_meta[m].get("genre", "")) for m in mids if m in movie_meta]
        rows.append({
            "person_a_id": pa, "person_b_id": pb,
            "collaboration_count": len(mids),
            "first_year": min(yrs) if yrs else None,
            "last_year":  max(yrs) if yrs else None,
            "shared_genres": ";".join(sorted(set(g for g in genres if g))),
        })
    return rows
...
def generate_media_links(movies: list, tv_series: list, cast_info: list,
                         movie_companies: list, episode_cast: list, rng) -> list:
    ...
    """
    if not tv_series or not cast_info:
        return []
    ...
    max_links = max(50, min(500, len(movies) // 100))
