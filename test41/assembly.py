...
def _build_actor_static_block(world: WorldState, concept: dict) -> ActorStaticBlock:
    ...
    if candidate_idx.size > min_pool and cache.ga_lower is not None:
        genre_mask = np.char.find(_to_str_ndarray(cache.ga_lower), genre_lower) >= 0
    ...
    if cache.ga_lower is not None:
        genre_match = np.char.find(_to_str_ndarray(cache.ga_lower[candidate_idx]), genre_lower) >= 0
    else:
    ...
    if hints:
        if cache.st_lower is not None:
            st_values = _to_str_ndarray(cache.st_lower[candidate_idx])
            tagged = np.array([f";{raw.replace(',', ';')};" for raw in st_values], dtype=object)
            style_match = np.zeros(n, dtype=bool)
            for hint in hints:
                if hint:
                    style_match |= np.char.find(_to_str_ndarray(tagged), f";{hint};") >= 0
    ...
    if cache.market_fit_lower is not None and movie_country:
        ...
        if target_market:
            mf = _to_str_ndarray(cache.market_fit_lower[candidate_idx])
...
def _pick_character_name(genre: str, archetype: str, slot: int, world: WorldState, gender: str = "M", used_names: set | None = None) -> str:
    ...
    def _select_from_indices(indices: np.ndarray, *, prefer_clean: bool) -> str | None:
        ...
        for idx in order:
            ...
            if candidate and candidate not in global_used and candidate not in used_names:
    ...
    if int(name_cache["size"]) > 0:
        base_mask = np.ones(int(name_cache["size"]), dtype=bool)
        key = genre.lower().strip()
        hints = _genre_archetype_candidates(world).get(key, [])
        if hints and bool(name_cache["has_archetype"]):
            hinted_mask = np.isin(name_cache["archetypes"], np.array(hints + [archetype], dtype=object))
            if int(np.count_nonzero(hinted_mask)) >= 20:
                base_mask &= hinted_mask
        elif bool(name_cache["has_archetype"]):
            archetype_mask = name_cache["archetypes"] == archetype
            if int(np.count_nonzero(archetype_mask)) > 0:
                base_mask &= archetype_mask
        if bool(name_cache["has_gender"]) and gender in ("M", "F"):
            gender_mask = name_cache["genders"] == str(gender).upper()
            if int(np.count_nonzero(base_mask & gender_mask)) >= 10:
                base_mask &= gender_mask

        pool_indices = np.flatnonzero(base_mask)
        ...
        if pool_indices.size != int(name_cache["size"]):
            all_indices = np.arange(int(name_cache["size"]), dtype=np.int32)
...
def _pick_cast_fast(world: WorldState, concept: dict, director_id: int, max_retries: int = 3) -> tuple[list[dict], list[tuple[int, int]]]:
    ...
    base_focus_cap = max(cast_shortlist_size * 16, 2200)
    ...
    for _attempt in range(max_retries):
        ...
        for slot in range(cast_size):
            ...
            usage_mult = _cast_usage_multiplier(
                film_count,
                slot,
                target_load=_prior_float(world, "target_actor_load", 6.0, lo=2.0, hi=20.0),
            )
            ...
            if slot == 0:
                focus_scores[unused_flags] *= max(1.40, float(cast_cfg["unused_actor_boost"]) * 0.90)
            elif slot == 1:
                focus_scores[unused_flags] *= max(1.55, float(cast_cfg["unused_actor_boost"]))
            else:
                focus_scores[unused_flags] *= max(1.75, float(cast_cfg["unused_actor_boost"]) * 1.10)
            if slot >= 1:
            ...
            if award_mask.any():
                focus_scores[award_mask[candidate_focus] & (focus_scores > 0)] *= float(cast_cfg["award_recent_boost"])
            if franchise_pool:
                ...
                focus_scores[franchise_pool_mask[candidate_focus] & (focus_scores > 0)] *= float(cast_cfg["franchise_pool_base_boost"]) + carryover_cast_bias
            if _policy_enabled(world):
                focus_scores *= policy_rule_mult[candidate_focus]
                if chemistry_target == "star_vehicle" and slot == 0:
                    focus_scores[static.top_star_mask[candidate_focus]] *= float(cast_cfg["star_vehicle_slot0_boost"])
                elif chemistry_target == "prestige_pairing" and slot <= 1:
                    focus_scores[np.isin(static.stage_vals[candidate_focus], np.array(["prime", "veteran", "legend"], dtype=object))] *= float(cast_cfg["prestige_pairing_boost"])
                elif chemistry_target == "volatile_ensemble" and slot >= 1:
                    focus_scores[np.isin(static.stage_vals[candidate_focus], np.array(["rising", "prime"], dtype=object))] *= float(cast_cfg["volatile_ensemble_boost"])
                elif chemistry_target == "balanced_ensemble" and slot >= 2:
                    focus_scores[np.isin(static.stage_vals[candidate_focus], np.array(["rising", "veteran"], dtype=object))] *= float(cast_cfg["balanced_ensemble_boost"])
            ...
            if ensemble.cast_ids:
                ...
                if ensemble.agencies:
                    ...
                    focus_scores[(focus_scores > 0) & agency_match] *= float(cast_cfg["agency_match_boost"])
                ...
                if ensemble.genders and static.actor_genders is not None:
                    ...
                    focus_scores[(focus_scores > 0) & gender_novel] *= float(cast_cfg["gender_novelty_boost"])
                if len(ensemble.nationalities) >= 2 and static.actor_nationalities is not None:
                    ...
                    focus_scores[(focus_scores > 0) & nat_novel] *= float(cast_cfg["nationality_novelty_boost"])
                if ensemble.tag_bitmasks and static.actor_tag_bitmasks is not None:
                    ...
                    if eligible.any():
                        ...
                        focus_scores[eligible & (max_jaccard > 0.5)] *= float(cast_cfg["tag_similarity_penalty"])
...
def _general_archetypes(world: WorldState) -> list[str]:
    ...
    normalized = _dedupe_strs(_canonical_archetype_name(value) for value in values)
    return normalized or list(_ARCHETYPE_GENERAL)
...
def _unique_archetypes(world: WorldState) -> set[str]:
    ...
    normalized = _dedupe_strs(_canonical_archetype_name(value) for value in values)
    return set(normalized) or set(_ARCHETYPE_UNIQUE)
...
def _career_archetype_bias(world: WorldState) -> dict[str, dict[str, float]]:
    ...
    return out or dict(_CAREER_ARCHETYPE_BONUS)
...
def _genre_archetype_bias(world: WorldState) -> dict[str, dict[str, float]]:
    ...
    return out or dict(_GENRE_ARCHETYPE_BONUS)
...
def _collaboration_style_archetype_bias(world: WorldState) -> dict[str, dict[str, float]]:
    ...
    return out or dict(_COLLAB_ARCHETYPE_BONUS)
...
def _generate_research_title(world: WorldState, genre: str, *, title_style: str = "") -> str:
    ...
    if best_title:
        return best_title
    raise RuntimeError(f"title grammar could not provide a research-valid title for genre={genre}")
