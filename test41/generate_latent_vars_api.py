...
def main():
    ...
    existing_ids = set()
    existing_latent = []
    if args.auto and PERSON_SHARD_DIR.exists() and not latent_path.exists():
        existing_latent = _load_json_shards(PERSON_SHARD_DIR)
    if latent_path.exists():
        ...
        existing_ids = {lv.get("person_id") for lv in existing_latent if isinstance(lv, dict)}
        if existing_latent:
            print(f"  Recovered {len(existing_latent)} person latent vars from shards")
    ...
    if len(persons_needing) == 0:
        ...
        if persons_needing:
            ...
            for batch_start in range(0, len(persons_needing), _batch_size):
                ...
                _write_jsonl_shard(PERSON_SHARD_DIR, batch_num, latent_vars)
    ...
    existing_cids = set()
    if args.auto and COMPANY_SHARD_DIR.exists() and not company_latent_path.exists():
        existing_company_latent = _load_json_shards(COMPANY_SHARD_DIR)
    if company_latent_path.exists():
        ...
        existing_cids = {cl.get("company_id") for cl in existing_company_latent if isinstance(cl, dict)}
    ...
    if len(companies_needing) == 0:
        ...
        if companies_needing:
            ...
            for batch_start in range(0, len(companies_needing), _batch_size):
                ...
                _write_jsonl_shard(COMPANY_SHARD_DIR, batch_num, latent_vars)
