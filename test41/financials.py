...
def _financial_config(world, key: str, default: Any) -> Any:
    ...
    if isinstance(default, Mapping):
        if isinstance(raw, Mapping):
            return _deep_merge_dict(default, raw)
        return deepcopy(dict(default))
    if isinstance(default, list):
        return deepcopy(raw if isinstance(raw, list) else default)
    return deepcopy(raw if raw is not None else default)
...
def _financial_required_float(
    ...
    return float(default_map.get(item_key, fallback))
