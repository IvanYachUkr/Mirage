...
class Step100ResumeManager:
    ...
    def prepare(self, *, run_payload: dict[str, Any], resume: bool, reset: bool) -> None:
    ...
    def restore_checkpoint(self, world, *, restore_graph: bool = True) -> dict[str, Any]:
        ...
        if persons_path.exists():
            world.persons = read_table(str(persons_path), "persons_enriched")
        if companies_path.exists():
            world.companies = read_table(str(companies_path), "companies_enriched")
    ...
    def commit_year(
        ...
        *,
        ...
        if graph is not None and hasattr(graph, "export_temporal_history"):
