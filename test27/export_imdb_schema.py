...
def convert(base_dir: Path, out_dir: Path, strict_job: bool, include_extras: bool):
    ...
    def _cast_note(desc: str, char_name: str, row_idx: int) -> str:
        ...
        h = row_idx * 2654435761 & 0xFFFFFFFF  # knuth hash
        frac = (h & 0xFFFF) / 0xFFFF

        if frac < 0.08:
            parts.append("(voice)")
        elif frac < 0.12:
            parts.append("(uncredited)")

        if desc and desc.lower() not in ("nan", "none", "unknown", ""):
            parts.append(f"(as {desc})" if frac < 0.20 else desc)
    ...
    def _mc_note(role: str, title_id, movie_df) -> str:
        ...
        if "production" in role_str:
            ...
            region_pool = ["(USA)", "(worldwide)", "(UK)", "(Germany)", "(Japan)",
    ...
    title = pd.concat([frame for frame in title_parts if frame is not None and not frame.empty], ignore_index=True)
    ...
    # Compute phonetic codes vectorized
    ...
    pcode_cf = []  # complete flattened name
    pcode_nf = []  # normal (first) name
    surname_pcode = []  # surname
    for n in person_names:
        first, last = _split_name(n)
        pcode_cf.append(_soundex(last + first))    # full flattened: last+first
        pcode_nf.append(_soundex(first))           # first name only
        surname_pcode.append(_soundex(last, 4))    # surname, shorter
    ...
    cast_info_actors = pd.DataFrame({
        ...
        "note": cast_work.apply(
            lambda row: _cast_note(row.get("character_description", ""), row.get("_char", ""), row.name),
            axis=1,
        ),
    ...
    cast_info = pd.concat([frame for frame in cast_parts if frame is not None and not frame.empty], ignore_index=True)
