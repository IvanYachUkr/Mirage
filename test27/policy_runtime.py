...
COMPARISON_REPORT_FILENAME = "test27_comparison_report.json"
...
def _strategy_catalog(genres: Sequence[str]) -> list[dict[str, Any]]:
    top_genres = list(genres[:6]) if genres else ["Drama", "Action", "Thriller", "Comedy"]
    second = top_genres[1] if len(top_genres) > 1 else top_genres[0]
    third = top_genres[2] if len(top_genres) > 2 else second
    return [
        {
            "strategy_tag": "prestige_drama",
            "label": "Prestige drama slate",
            "genre_focus": normalize_counter_weights({"Drama": 5, "Romance": 2, "Mystery": 1}, allowed=top_genres),
        ...
        },
        {
            ...
            "genre_focus": normalize_counter_weights({top_genres[0]: 3, second: 3, "Sci-Fi": 3, "Fantasy": 3, "Action": 3}, allowed=top_genres),
        ...
        },
        {
            ...
            "genre_focus": normalize_counter_weights({"Horror": 3, "Thriller": 3, third: 2, "Sci-Fi": 2}, allowed=top_genres),
        ...
        },
        {
            ...
            "genre_focus": normalize_counter_weights({"Comedy": 3, "Animation": 3, "Fantasy": 2, "Drama": 1}, allowed=top_genres),
...
def _default_bucket_bias(
    ...
    *,
    ...
    lo = safe_int(bucket.get("start_year"), 1975)
    genre_bias = {genre: 1.0 for genre in genres}
    if lo < 1990:
        for genre in ("Drama", "Crime", "War"):
            if genre in genre_bias:
                genre_bias[genre] += 0.35
    elif lo < 2005:
        for genre in ("Action", "Romance", "Thriller"):
            if genre in genre_bias:
                genre_bias[genre] += 0.28
    else:
        for genre in ("Sci-Fi", "Fantasy", "Animation", "Thriller"):
            if genre in genre_bias:
                genre_bias[genre] += 0.30

    country_bias = {country: 1.0 for country in countries}
    for country in countries:
        market = infer_market(country)
        if market == "North America":
            country_bias[country] += 0.18
        elif market == "Europe":
            country_bias[country] += 0.08
        elif market == "Asia":
            country_bias[country] += 0.12

    market_counter: Counter[str] = Counter(infer_market(country) for country in countries)
    market_bias = {market: 1.0 for market in market_counter}
    if lo >= 2000 and "Global" in market_bias:
        market_bias["Global"] += 0.12
    return (
        normalize_counter_weights(genre_bias, allowed=genres),
        normalize_counter_weights(country_bias, allowed=countries),
        normalize_counter_weights(market_bias),
    )
...
def build_default_world_policy(
    *,
    ...
    strategies = _strategy_catalog(genres)
    ...
    for bucket in bucket_defs:
        genre_bias, country_bias, market_bias = _default_bucket_bias(bucket, genres=genres, countries=countries)
        year_buckets.append(
            {
                ...
                "franchise_pressure": 0.22 if int(bucket["start_year"]) < 1995 else 0.34,
                "sequel_pressure": 0.18 if int(bucket["start_year"]) < 1995 else 0.30,
            }
        )
...
def normalize_world_policy(
    ...
    *,
    ...
    if isinstance(raw_country_market, Mapping):
        for country in countries:
            country_market_map[country] = str(raw_country_market.get(country, country_market_map.get(country, infer_market(country))))
    ...
    for bucket in bucket_rows:
        ...
        for _score, slot in bucket_slots:
            if len(selected_bucket_slots) >= per_bucket_cap:
                break
            country = str(slot.get("country", ""))
            if not country or country_counts[country] >= max_per_country:
                continue
            slot_key = (
                str(slot.get("genre", "")),
                str(slot.get("tier", "")),
...
def _default_pack(slot: Mapping[str, Any], idx: int) -> dict[str, Any]:
    ...
    return {
        ...
        "country": country,
        "market": market,
        "premise_archetype": f"{genre.lower()} world under pressure",
        ...
        "keyword_seed_cluster": [genre.lower(), market.lower().replace(" ", "_"), tier.lower()],
...
def _default_priority_motifs_for_bucket(genres: Sequence[str], market: str, tier: str) -> list[str]:
    ...
    if tier in {"Epic", "A"}:
        base.extend(["scale_event", "franchise_pressure"])
    elif tier in {"Indie", "Micro"}:
        base.extend(["character_intimacy", "discovery_impulse"])
    else:
        base.extend(["ensemble_pressure", "market_balance"])
...
def _default_motif_drift(bucket_start_year: int, genre: str) -> list[str]:
    year = int(bucket_start_year)
    if year < 1990:
    ...
    for bucket in year_buckets:
        ...
        for market in markets:
            for tier in tiers:
                tier_name = str(tier)
                release_season_bias = (
                    "summer" if tier_name in {"Epic", "A"} else
                    "awards" if tier_name in {"Indie", "Micro"} and any(g in {"Drama", "Documentary"} for g in top_genres) else
                    "fall"
                )
                slates.append(
                    {
                        ...
                        "priority_motifs": _default_priority_motifs_for_bucket(top_genres, str(market), tier_name),
                        "motif_drift": _default_motif_drift(start_year, top_genres[0]),
                        "release_pressure": round(0.45 + (0.18 if tier_name in {"Epic", "A"} else 0.0), 3),
                        "release_season_bias": release_season_bias,
                        "sequel_appetite": round(min(1.0, sequel_pressure + (0.12 if tier_name in {"Epic", "A"} else -0.05)), 3),
                        "novelty_target": round(0.32 + (0.18 if tier_name in {"Indie", "Micro"} else 0.04), 3),
                        "company_strategy_bias": strategies[:3] if strategies else ["genre_lab"],
                    }
                )
    return {
        "meta": {
...
def build_default_franchise_bibles(
    ...
    for idx, franchise in enumerate(rows, start=1):
        ...
        name = str(franchise.get("name") or f"Franchise_{franchise_id}")
        bibles.append(
            {
...
def normalize_franchise_bibles(
    ...
    *,
    ...
    fallback = build_default_franchise_bibles(franchises)
    bible_by_id = {
    ...
    if isinstance(raw_bibles, list):
        for idx, raw in enumerate(raw_bibles, start=1):
            ...
            if not bible:
                bible = {
                    "franchise_id": franchise_id,
                    "franchise_name": str(raw.get("franchise_name") or f"Franchise_{franchise_id}"),
