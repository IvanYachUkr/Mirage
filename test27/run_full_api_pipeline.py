...
def _build_runner_env(
    *,
    ...
    if args.llm_max_attempts is not None:
        ...
        env["DATA_SYS_LLM_MAX_ATTEMPTS"] = "2"
...
def main() -> None:
    ...
    print("=" * 72)
    print("TEST27 HIGH-LEVEL API RUNNER")
    print("=" * 72)
    ...
    print(f"Compare runs:  {not bool(args.skip_compare)}")
    ...
    if not bool(args.skip_compare):
        compare_cmd = [
            python,
            str(base_dir / "compare_test25_test27.py"),
            "--baseline",
            str(baseline_dir),
            "--variant",
            str(base_dir),
        ]
        _run_command(compare_cmd, cwd=base_dir, log_path=log_path, env=runner_env)
    ...
    print(f"Log file: {log_path}")
    print(f"Usage log: {usage_log}")
    print(f"Comparison report: {base_dir / 'test27_comparison_report.json'}")
    print("=" * 72)
