...
from policy_runtime import infer_market, resolve_company_strategy, safe_load_json, world_policy_path
...
def generate_person_person_edges(persons, latent_map, rng, max_edges_per_person=None, world_policy: dict[str, Any] | None = None):
    ...
    for pg, bucket_idxs in genre_buckets.items():
        ...
        genre_bonus = np.array([_policy_genre_strength(world_policy or {}, pg_list[int(idx)]) for idx in bidxs], dtype=np.float32)
        raw_score = (
            0.40 * style_sim +
            0.25 * jaccard +
            0.15 * risk_match +
            ...
            0.10 * noise +
            0.03 * ((genre_bonus[:, None] + genre_bonus[None, :]) / 2.0)
        )
...
def generate_company_company_edges(companies, company_latent, rng, world_policy: dict[str, Any] | None = None):
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            ...
            if genre_overlap * policy_mult > 0.5 and tier_overlap > 0.5:
                edges.append({
                    "src_id": cid_a,
                    "dst_id": cid_b,
                    ...
                    "sign": "-",
                    "weight": round(genre_overlap * tier_overlap * min(1.2, policy_mult), 2),
                    "source_kind": "latent_hybrid",
            ...
            elif genre_overlap * policy_mult > 0.3 and tier_overlap < 0.3:
                edges.append({
                    "src_id": cid_a,
                    "dst_id": cid_b,
                    "src_name": c_a.get("name", "?"),
                    ...
                    "sign": "+",
                    "weight": round(genre_overlap * 0.7 * min(1.2, policy_mult), 2),
                    "source_kind": "latent_hybrid",
...
def main(base_dir: str | Path | None = None, *, force_legacy: bool = False, use_world_policy: bool = False):
    ...
    if company_latent:
        ...
        pc_edges = generate_person_company_edges(
            graph_persons, companies, person_latent, company_latent, rng, world_policy=world_policy
        )
    ...
    if company_latent:
        ...
        cc_edges = generate_company_company_edges(companies, company_latent, rng, world_policy=world_policy)
